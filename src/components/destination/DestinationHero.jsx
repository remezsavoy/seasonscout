...
export function DestinationHero({ destination, topRightAction = null }) {
  const backgroundStyle = destination.heroImageUrl
    ? {
        backgroundImage: `url(${destination.heroImageUrl})`,
      }
    : undefined;
      ...
      <section className={sharedHeroImageContainerClassName}>
        ...
        <div className="relative grid gap-10 px-6 py-8 text-white sm:px-8 sm:py-10 lg:grid-cols-[1.08fr,0.92fr] lg:px-12 lg:py-14">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 font-display text-5xl leading-tight drop-shadow-md sm:text-6xl">{destination.name}</h1>
            <p className="mt-3 text-lg capitalize text-white/72 drop-shadow-md">
              {destination.country}
              {destination.continent ? ` / ${destination.continent}` : ''}
            </p>
            <p className="mt-6 max-w-2xl text-base leading-8 text-white/84 drop-shadow-md">{destination.summary}</p>
